...
class TgtgClient(AsyncResource):
    ...
    APP_VERSION: ClassVar[Version] = Version("25.4.11")
    ...
    async def _post(self, endpoint: TgtgApi, *path_params: str | int, json: JSON | None = None) -> JSON:
        ...
        if endpoint in {TgtgApi.FAVORITES, TgtgApi.ITEMS, TgtgApi.ITEM_STATUS}:
            local_offset = SystemDateTime.now().offset
            headers["X-24HourFormat"] = "false"
            headers["X-TimezoneOffset"] = (
                "{:+03}:{:02}".format(*local_offset.in_hrs_mins_secs_nanos()) if local_offset else "Z"
            )
        r = await self._httpx.post(
    ...
    async def get_favorites(self, pages: Iterable[int] | None = None) -> list[Item]:
        return [f async for f in self._get_favorites(pages)]

    async def _get_favorites(self, pages: Iterable[int] | None = None) -> AsyncGenerator[Item]:
        PAGE_SIZE = 50
        if pages is None:
            pages = count()
    ...
    async def pay(self, reservation: Reservation, voucher: MultiUseVoucher | None = None) -> list[Payment]:
        if voucher is None:
            ...
            voucher = max(vouchers, key=lambda v: v.amount.minor_units)
